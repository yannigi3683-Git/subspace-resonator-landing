...
- **The crowd band is squeezed between two obstacles, and both must be honoured.** Below it sits the volume pill (`absolute bottom-4 left-4` in `LiveRoom.tsx`), which covered the bottom-left avatars and the `+N in the crowd` line on mobile. Above it sits `PsyViz` (`top-[52%]`, `64vmin` capped at 460px), which is a large share of a phone screen. Lifting the band by `CONTROLS_GUTTER_PX` to clear the pill pushed its top row **into the visualizer** on both phone and desktop — honouring one obstacle put avatars inside the other. `bandPx` now derives the band from both: bottom at `boxH - CONTROLS_GUTTER_PX`, top at `max(naturalTop - gutter, vizBottomPx + clearance)`. `vizBottomPx` mirrors the visualizer's geometry from its JSX wrapper, so **if that wrapper's position or size changes, those constants must change with it**. A test asserts every tile at full capacity clears both edges, wander included.
