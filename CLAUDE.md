...
- **The guest room drifts too, and it is fixed by the SAME observer (2026-08-22).** On the public broadcast the host console read 21 and the guest dance floor read ~21 while the server held **12** - two different browsers, two machines, wrong by the same +9, because neither had re-pruned since page load. So "the room is correct" (2026-08-21) was a freshly-opened tab, not a property. Both callers now read the roster from `usePresenceObserver` (`src/radio/hooks/usePresenceObserver.ts`), which also owns `dedupeByDevice`. `usePresence` keeps its own channel purely to `track()`, handle rename/cheer, and watch `kicks`/`bans`; its three presence bindings are now no-ops but **must stay**, because supabase-js only enables presence on a channel that has presence bindings and `track()` needs it. **The guest channel must never be rebuilt to re-prune:** it tracks, so a rebuild broadcasts leave+join to the whole room every cycle (a presence storm at crowd scale) and re-seeds the leaver's avatar at a new random position on every other screen, since `useCrowdMotion` keys agents by uid. The observer never tracks, so its rejoin is invisible. Cost of the fix: each listener now holds a second Realtime connection. It costs **no extra anonymous sign-in** - the observer is fed the existing session's token via `setAuth` - so the documented 300/h-per-IP capacity ceiling is unchanged.
