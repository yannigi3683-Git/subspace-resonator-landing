...
- **The crowd ROAMS a region; there is no grid (`feat/crowd-roam`, 2026-08-02).** It used to be a grid of fixed cells inside the bottom ~25% strip, each avatar drifting only the slack inside its own cell (about 2px at density). That read as a formation, and nobody could stand near the PA stacks because there was no floor under them. Now `crowdRegion()` is the whole usable floor (below the banner/DJ, above the controls gutter, full width), `vizCircle()` is an exclusion circle the crowd walks *around*, and `roamPath()` gives each listener a deterministic looping tour seeded from their uid, rejection-sampled clear of the visualizer. `radio-roam` in `radio.css` walks the waypoints via `--r1x..--r4y`, so 200 roamers still cost no JS per frame. **The animation interpolates in a STRAIGHT LINE between waypoints, so clearing the circle at each waypoint is not enough** - two waypoints on opposite sides walked the avatar straight through the artwork. `roamPath` also requires every leg (including the closing leg home) to clear the circle, and the test samples along each leg rather than only at its ends. A tile's position comes from waypoint 0, NOT a grid cell: with a grid anchor the crowd snapped back into formation every time the loop came round. `crowdCapacity` and `crowdTileSize` derive from region area minus the visualizer, so density and label visibility still fall off as the room fills.
- **The no-overlap guarantee is deliberately retired.** `gridSlot`, `computeGridDims`, `bandPx` and the footprint-overlap tests are gone with the grid. Roaming avatars cross each other, which is what a crowd does, and the owner had already chosen "a packed room should look packed". What replaced it, and what the tests now assert, is: every waypoint is inside the region and outside the visualizer, paths are deterministic per uid, and the median tour actually crosses a quarter of the floor rather than shuffling in place.
