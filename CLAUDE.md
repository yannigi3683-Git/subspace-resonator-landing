...
**A stuck DEEP BUFFER `STALLED` badge is the SAME bug, one layer up (2026-08-21).** Reported the same afternoon: the badge read STALLED while the deep buffer was provably healthy (media sequence 604 -> 607 -> 610 in real time, `Access-Control-Allow-Origin: *`, `Cache-Control: max-age=1`). `useStation` read the row once at mount and then depended entirely on a `postgres_changes` subscription that is never rebuilt (deps `[supabase]`, no re-read, no fallback). The host reconnected at 11:52, the server wrote a new `cfSessionId` + `streamUrl`, the console never received that UPDATE, and `useDeepBufferStatus` went on probing the PREVIOUS session's playlist. **R2 still serves it, frozen** (`Last-Modified: 11:52:06`, byte-identical 20s apart) because nothing deletes old prefixes, so the probe's "body unchanged for 15s" rule is satisfied forever. Deducible without a browser: the new URL is live and CORS-open, so a console reading ON would be impossible to confuse with STALLED, which means the console does not hold the new URL. Note the failure mode of the probe itself: `nextProbeState` treats a failed fetch as "no change", so **repeated fetch failures also read STALLED** - a stuck badge means "the URL I am watching is not moving", never "the restreamer is dead". Fix: `useStation` now re-reads the row every 60s while the tab is visible, and again on `visibilitychange`. A plain re-read, not a channel rebuild - no teardown, no socket effects, one indexed single-row select, and it protects listeners as well as the console. **One dead Realtime feed on the console produced both symptoms** (frozen listener count and stuck STALLED badge); if one is seen, check the other.

**The host console's listener count froze on long shows, and the fix is a rebuilt observer (`fix/console-presence-freeze`, 2026-08-21).** Measured mid-broadcast, all three numbers at one instant: server truth 3, guest room 3, host console **7+**. `AdminConsole` subscribed to `room:main` once with deps `[supabase]` and held that channel for the whole show. A presence map is one `presence_state` snapshot plus accumulated `presence_diff`, and **only a fresh `presence_state` prunes** — so a channel that never reconnects can never re-prune. Miss one diff, or get dropped server-side, and the number is stuck for the rest of the broadcast. **The observable is an over-count that never self-corrects; whether the number still MOVES depends on how dead the channel is.** 2026-08-21 it sat stuck (console 7+, server 3). 2026-08-22, mid public broadcast, it moved the whole time (20 -> 22 -> 21 -> 24 -> 22) while server truth held at 14-15 - so diffs were still arriving and only the leaves were being lost. A lossy channel LEAKS, a dead one FREEZES, and one rebuild cures both because both are just "no second `presence_state` ever arrives". Do not use "is the number moving?" to rule this out. Likely trigger: `room:main` is `private: true`, so Realtime re-authorizes it against the JWT, and the admin token expires at 1h while the console tab lives for hours. The old `.subscribe()` had no status callback, so the drop was silent. Now the observer rebuilds every 60s, re-auths from `authToken()` first, and logs any non-`SUBSCRIBED` status. **This does not contradict the 2026-08-16 investigation** (`docs`/memory: presence self-heals across an outage, 5/5 runs) — that disproof covered healing *after a reconnect* and is still correct; the never-reconnects case was simply never tested. The guest room stays correct by accident: `usePresence`'s effect deps include `identity.name/avatarId/deviceId/position`, so a rename rebuilds its channel. If a listener tab is ever seen frozen, apply the same pattern there.
- **The guest room drifts too, and it is DELIBERATELY LEFT THAT WAY (decided 2026-08-23).** On the public broadcast the host console read 21 and the guest dance floor read ~21 while the server held **12** - two different browsers, two machines, wrong by the same +9, because neither had re-pruned since page load. The guest half of the fix was built, tested and then **removed**: only a fresh `presence_state` prunes, so staying accurate means rejoining, and rejoining a channel that `track()`s costs either a room-wide leave+join storm every cycle or a **second Realtime client per listener**. Supabase Free allows **200 concurrent clients total** (dashboard, 2026-08-23), so two per listener would have halved the room from ~200 people to ~100 - to fix a cosmetic number. Capacity wins. A listener can refresh the page to recount; the **host cannot** (that tab owns the publisher), which is why `AdminConsole` alone pays for an observer, costing exactly one extra client for the whole broadcast. `usePresence` therefore keeps reading its own channel and importing `dedupeByDevice` from `usePresenceObserver`. Two tests lock this in: `usePresence` must open **exactly one** channel, and `AdminConsole` must never open one on the shared prop client. Revisit only on Pro, or with a design that costs no extra client.
- **Drift is PER TAB and grows with tab age, which is why the console is always the worst offender.** Each tab holds its own channel, misses its own leaves, and never recounts, so two tabs on the same machine disagree with each other and with the server by different amounts. The console tab is always the oldest: it is opened first to go live and then must never be reloaded. Observed across both broadcasts: 2026-08-21 console 7 vs guest 3 vs server 3; 2026-08-22 console 21 vs guest ~21 vs server 12, by which point both tabs were hours old. A guest refresh therefore changes the number only *sometimes*, because a young tab that has not yet missed a leave is already right. This **supersedes** the earlier guess that the guest room stays correct because renames rebuild its channel; tab age explains every observation without it.
- **The one-second diagnostic: REFRESH THE GUEST PAGE.** Observed 2026-08-22 - a reload makes the listener number correct immediately. That is the whole bug in one gesture: a new page means a new channel, which means a fresh `presence_state`, which is the only thing that prunes. A long-lived tab drifts; a new tab is right. So "does a refresh fix it?" separates this bug from a genuine miscount faster than any probe, and it is what `usePresenceObserver` automates - it refreshes the roster's channel every 60s instead of making the user reload. **Never do this on `radio.html#admin` while on air**: that tab owns the publisher and a reload ends the broadcast. A synthetic reproduction failed and is worth not repeating: hard-killing three fakes' sockets without an untrack did NOT make a long-lived observer leak, because the server does emit a leave on socket death. The trigger is still unidentified; the fix is a convergence guarantee that holds whatever the trigger is.
- **A refresh only fixes the tab you refresh, and the host cannot use it.** The console is a separate tab with its own long-lived channel, so reloading `/radio` leaves the console's badge exactly as wrong as before - and the console cannot be reloaded at all while on air, because that tab owns the publisher. So until the observer fix ships the host has NO in-console workaround. The two safe readings mid-show are a freshly refreshed `/radio` tab (the host is already in it to moderate) and `scripts/radio-presence-probe.mjs`. **Do not open a second `radio.html#admin` tab to read the count**: it would be correct, but it also renders GO LIVE and END BROADCAST against an already-live station row, and one stray tap ends the show.
- **Verifying a count claim: two independent fresh observers at one instant.** Run on 2026-08-22 when the UIs disagreed with the probe: both fresh sockets returned 12 with byte-identical rosters, which is what rules out "the probe is the broken one". Cross-checking chat is the second test - 7 of 8 people who had spoken in the previous 25 minutes were in the probe's roster, and the one missing had been quiet 17 minutes. Note the honest limit of ALL of this: presence measures "has a live Supabase connection", not "is hearing audio", and the two genuinely diverge because audio rides Cloudflare and never touches Supabase.
- **The observer runs on its OWN Supabase client**, built by the shared `makeSupabase()` factory now in `src/radio/supabaseClient.ts` (extracted from `RadioApp.tsx`, same BOM-sanitising `cleanFetch`). Not a style choice: the host tab's main client also carries `room:control` and `room:nowplaying`, which **do** reach listeners, and a separate socket makes "a rebuild cannot disturb them" true by construction instead of by argument. That second client is created with `persistSession:false`, `autoRefreshToken:false` and its **own `storageKey`** — two GoTrue clients sharing the default key on one origin both refresh the same session and can knock the host out of the console.
- **Rebuild order is remove-then-create.** `supabase-js` `channel(topic)` returns the *existing* channel for a matching topic (`RealtimeClient.js` `channel()`), so building the replacement first just hands back the stale one. Nothing clears `listenerCount` on teardown either: the badge animates on `key={listenerCount}`, so a flash of 0 would fire that animation every minute.
