...
- **The dance floor's crowd cap is measured, not fixed (`crowdCapacity` in `DanceFloor.tsx`).** It was a constant `CROWD_RENDER_CAP = 30`, which is roughly what a phone's band holds, so a wide desktop window binned most of the room into `+N in the crowd` for no reason. Capacity is now `floor(bandW / MIN_CELL) * floor(bandH / MIN_CELL)` where `MIN_CELL = AVATAR_MIN / 0.6` is the smallest cell an avatar fits without the size clamp forcing an overlap, hard-capped at 200 for DOM sanity. The `crowdCapacity` tests assert no two footprints overlap at full capacity on phone, tablet and desktop boxes.
- **The crowd band is lifted clear of the volume pill, not shrunk.** The pill is `absolute bottom-4 left-4` in `LiveRoom.tsx`, over the same bottom strip the crowd occupies, and it covered the bottom-left avatars and the `+N in the crowd` line on mobile. `bandPx` moves the band up by `CONTROLS_GUTTER_PX`. Shrinking it instead was tried first and cost the name labels for ordinary small crowds on a phone (two rows in 86px instead of 150px, so no cell was tall enough for a label).
- **Testing host UI with a real crowd:** `node --env-file=.env scripts/radio-fake-crowd.mjs <count>` joins N fake listeners to presence (anon sign-in → `room:main` → `track()`), holds them until Ctrl+C, then untracks. Distinct from `radio-loadtest.mjs`, which measures the sign-in ceiling and never joins presence, so it puts nobody in the roster. **`room:main` is one channel for the whole Supabase project** — not per broadcast, not per deployment — so fakes appear in the production room to any real listener. Run it only on an empty room, and delete the throwaway anon users afterwards. A second arg offsets the `Test NN` names (`... 60 60` adds Test 61-120 beside a running `... 60`) so a crowd can be grown from a second window without re-spending the sign-ins already paid to the per-IP cap. `--chat` additionally makes the fakes talk: real `chat_messages` rows, so they need the station LIVE and unlocked (off air `chat_allowed()` refuses them with `23514`), they keep the 48h TTL, and **they land in the END BROADCAST transcript** — never use it on a broadcast whose log matters.
