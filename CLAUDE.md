...
**`/radio?debug` is the listener-side diagnostic, and it is what actually settled all of this.** A hidden pill shows `TRANSPORT:` (`WEBRTC`, `WEBRTC -> HLS`, `CROSSFADING...`, or green `HLS (deep buffer)`), `HLS-BUF` seconds, plus WebRTC stats. Screenshots of that pill are better evidence than any reasoning from source: `HLS-BUF 0.0s` on `WEBRTC -> HLS` is a dead hls.js, and a healthy `HLS-BUF` sitting on `WEBRTC -> HLS` is the crossfade refusing to re-arm. Ask for it before theorising.

**The `TAP / REBUILDS / PLAYING / ERR` line on that pill is PERMANENT - do not remove it as "temporary instrumentation".** It was added 2026-09-10 to investigate a tap that sometimes did nothing after a listener's own network dropped, and the commit that introduced it called it temporary. Yanni decided to keep it: the underlying cause was never reproduced with instrumentation attached, so if it recurs this line answers in one glance what previously cost an afternoon and three wrong explanations. It is inert - the whole pill is gated behind `showDebug`, true only when the URL carries `?debug`, so no listener has ever seen it.

**Headless browsers cannot test this.** A Playwright run joins the room fine but has no audio device, so WebRTC and hls.js never really play and the deep buffer never forms - the exact thing that must be broken and watched recover. Automated end-to-end verification of listener audio is not available; use a real phone and the debug pill.
...
**THE CONSOLE STATE-FREEZE BUG (2026-08-21 to 2026-08-23, `fix/console-presence-freeze`).** Read this whole block before touching presence or `useStation`. The notes are in the order the thing was actually understood, and later ones supersede earlier ones.

**What it is.** Every screen showing room state builds it from ONE snapshot taken at mount plus a stream of deltas, and **only a fresh `presence_state` (or a fresh row read) prunes**. A subscription that is never rebuilt therefore has no way back to truth: miss one delta and the error is permanent. **The observable is an over-count that never self-corrects, and whether the number still MOVES depends on how dead the channel is** - a lossy channel LEAKS, a fully dead one FREEZES, and one rebuild cures both because both reduce to "no second snapshot ever arrives". **Never use "but the number is still moving" to rule this out.** Likely trigger: `room:main` is `private: true`, so Realtime re-authorizes it against the JWT, and the admin token expires at 1h while the console tab lives for hours; the old `.subscribe()` had no status callback, so the drop was silent. **This does not contradict the 2026-08-16 investigation** (presence self-heals across an outage, 5/5 runs): that disproof covered healing *after a reconnect* and is still correct, and the never-reconnects case was simply never tested. **One dead Realtime feed produced BOTH symptoms below; if you see one, check the other.**

- **Symptom 1, the listener count (2026-08-21, again 2026-08-22).** 2026-08-21, all three numbers at one instant: server truth 3, guest room 3, host console **7+**, and it sat stuck. 2026-08-22 mid public broadcast it MOVED the whole time (20 -> 22 -> 21 -> 24 -> 22) while server truth held at 14-15, so diffs were arriving and only leaves were being lost. `AdminConsole` had subscribed to `room:main` once with deps `[supabase]` and held that channel all show.
- **Symptom 2, a stuck DEEP BUFFER `STALLED` badge (2026-08-21).** The badge read STALLED while the deep buffer was provably healthy (media sequence 604 -> 607 -> 610 in real time, `Access-Control-Allow-Origin: *`, `Cache-Control: max-age=1`). `useStation` read the row once at mount and then depended entirely on a `postgres_changes` subscription that is never rebuilt. The host reconnected at 11:52, the server wrote a new `cfSessionId` + `streamUrl`, the console never received that UPDATE, and `useDeepBufferStatus` went on probing the PREVIOUS session's playlist. **R2 still serves it, frozen** (`Last-Modified: 11:52:06`, byte-identical 20s apart) because nothing deletes old prefixes, so "body unchanged for 15s" is satisfied forever. Deducible without a browser: the new URL is live and CORS-open, so a console holding it could not read STALLED. **Note the probe's own failure mode:** `nextProbeState` treats a failed fetch as "no change", so **repeated fetch failures also read STALLED**. A stuck badge means "the URL I am watching is not moving", never "the restreamer is dead".
- **Drift is PER TAB and grows with tab age, which is why the console is always the worst offender.** Each tab holds its own channel, misses its own leaves, and never recounts, so two tabs on the same machine disagree with each other and with the server by different amounts. The console tab is always the oldest: opened first to go live, then never reloadable. 2026-08-21 console 7 vs guest 3 vs server 3; 2026-08-22 console 21 vs guest ~21 vs server 12, by which point both tabs were hours old. **This SUPERSEDES the earlier guess** that the guest room stays correct because renames rebuild its channel: tab age explains every observation without it.
- **The one-second diagnostic: REFRESH THE GUEST PAGE.** Observed 2026-08-22, a reload makes the number correct immediately, because a new page means a new channel means a fresh `presence_state`. So "does a refresh fix it?" separates this bug from a genuine miscount faster than any probe, and it changes the number only *sometimes*, since a young tab that has not yet missed a leave is already right. **Never do this on `radio.html#admin` while on air:** that tab owns the publisher and a reload ends the broadcast. A synthetic reproduction **failed** and is worth not repeating: hard-killing three fakes' sockets without an untrack did NOT make a long-lived observer leak, because the server does emit a leave on socket death. **The trigger is still unidentified; the fix is a convergence guarantee that holds whatever it turns out to be.**
- **A refresh only fixes the tab you refresh, and the host cannot use it.** Reloading `/radio` leaves the console exactly as wrong as before. Until the observer shipped, the host had NO in-console workaround. The two safe mid-show readings are a freshly refreshed `/radio` tab (the host is already in it to moderate) and `scripts/radio-presence-probe.mjs`. **Do not open a second `radio.html#admin` tab to read the count:** it would be correct, but it renders GO LIVE and END BROADCAST against an already-live station row, and one stray tap ends the show.
- **Verifying a count claim: two independent fresh observers at one instant.** Run 2026-08-22 when both UIs disagreed with the probe: both fresh sockets returned 12 with byte-identical rosters, which is what rules out "the probe is the broken one". Cross-checking chat is the second test: 7 of 8 people who had spoken in the previous 25 minutes were in the probe's roster, and the one missing had been quiet 17 minutes. **Honest limit of all of it:** presence measures "has a live Supabase connection", not "is hearing audio", and the two genuinely diverge because audio rides Cloudflare and never touches Supabase.
- **What shipped: rebuild the console's roster channel, re-read the station row.** `usePresenceObserver` rejoins `room:main` every 60s, re-auths from `authToken()` first, and logs any non-`SUBSCRIBED` status. `useStation` re-reads its row every 60s while the tab is visible and again on `visibilitychange` - a plain indexed single-row select, not a channel rebuild, so no teardown and no socket effects, and it protects listeners as well as the console (it is also how they learn mode, title, slow-mode and lock). **The observer runs on its OWN Supabase client**, from the shared `makeSupabase()` factory in `src/radio/supabaseClient.ts` (extracted from `RadioApp.tsx`, same BOM-sanitising `cleanFetch`), with `persistSession:false`, `autoRefreshToken:false` and its **own `storageKey`** - the host tab's main client also carries `room:control` and `room:nowplaying` which **do** reach listeners, so a separate socket makes "a rebuild cannot disturb them" true by construction, and two GoTrue clients sharing the default key would both refresh the same session and could knock the host out of the console. **Rebuild order is remove-then-create:** `supabase-js` `channel(topic)` returns the *existing* channel for a matching topic, so building the replacement first just hands back the stale one. Nothing clears `listenerCount` on teardown either, since the badge animates on `key={listenerCount}` and a flash of 0 would fire that animation every minute.
- **VERIFIED ON A REAL LONG BROADCAST (2026-09-11).** The check the console fix was still owed. A broadcast 12h27m old, on the original console tab (never reloaded, the exact condition that produced the bug), read against `radio-presence-probe.mjs` at the same instant: console **4**, server **4**; then a listener left and both fell to **3**. The falling half is the part that matters - the old failure was **missed leaves**, a number that only ever climbed, so a leave tracked after 12.5 hours is what proves the 60s rejoin is pruning. Also clean through that run: `startedAt` and `cfSessionId` both unchanged since go-live (zero host reconnects), deep buffer live at segment 11,200, `API ALIVE`.

- **What deliberately did NOT ship: the same fix for guests (decided 2026-08-23).** It was built, tested, and **removed**. Rejoining a channel that `track()`s costs either a room-wide leave+join storm every cycle or a **second Realtime client per listener**, and Supabase Free allows **200 concurrent clients total**, so two per listener would have halved the room from ~200 people to ~100 to fix a cosmetic number. Capacity wins. A listener can refresh to recount; the **host cannot**, which is why `AdminConsole` alone pays for an observer, costing exactly one extra client for the whole broadcast. `usePresence` therefore keeps reading its own channel and imports `dedupeByDevice` from `usePresenceObserver`. **Two tests lock this in:** `usePresence` must open **exactly one** channel, and `AdminConsole` must never open one on the shared prop client. Revisit only on Pro, or with a design costing no extra client.
- **Presence never touches audio.** `src/radio/rtc/*` contains zero Supabase imports (the only match is an error *string* in `publisher.ts`); audio is the Cloudflare SFU plus HTTP signalling. That is what makes presence work safe to change without a device test.
- **`scripts/radio-presence-probe.mjs` is the referee.** Subscribes to `room:main` and never calls `track()`, so it is invisible and safe to run during a live show; costs one anon sign-in per run. It prints server truth to compare against both UIs **at the same instant** — a churning room makes two glances a minute apart disagree honestly. **Its presence handlers are load-bearing even though they are empty:** supabase-js only enables presence on a channel with presence bindings, so without them `presenceState()` silently returns `{}` and the probe reports an empty room no matter who is in it.

**Realtime limits, read off the dashboard 2026-08-23 (Free plan).** `Max concurrent clients` **200**, `Max events per second` 100, `Max presence events per second` **20**, max payload 256KB. The last three are **Pro-locked**, so treat them as constants to design around, not knobs. Two consequences that are easy to miss: every listener holds one client, so **anything that adds a second client per listener halves how many people fit in the room**; and CHEER sends a presence event rate-limited to one per device per 3s, so roughly **60 people cheering flat out saturates the 20/s presence budget**. These sit alongside the anon sign-in cap below, which is a different quota entirely - a change can leave sign-ins untouched and still halve the room.

**Rule for proposing ANY radio change: lead with its effect on (1) broadcast quality and stability and (2) crowd size on the free plan.** Not as a caveat, not when asked. This exists because the guest-side count fix above was built and nearly shipped without anyone noticing it halved the listener ceiling; it was caught only because Yanni asked "will it reduce my capacity?". That should never be the owner's job to ask. If a change trades away either one, do not offer it as the default, and say plainly what is being traded.
