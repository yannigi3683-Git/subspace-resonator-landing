...
- **Testing host UI with a real crowd:** `node scripts/radio-fake-crowd.mjs <count>` joins N fake listeners to presence (anon sign-in → `room:main` → `track()`), holds them until Ctrl+C, then untracks. Distinct from `radio-loadtest.mjs`, which measures the sign-in ceiling and never joins presence, so it puts nobody in the roster. **`room:main` is one channel for the whole Supabase project** — not per broadcast, not per deployment — so fakes appear in the production room to any real listener. Run it only on an empty room, and delete the throwaway anon users afterwards.
