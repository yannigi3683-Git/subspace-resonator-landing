...
- **Crossings are prevented by TERRITORIES, not collision code.** `territories()` divides the region into a square-ish grid, drops any cell touching a fixed prop, and gives each listener one disc inscribed in their cell (`PATCH_FILL = 0.94`, so neighbours keep a gap instead of being exactly tangent - at tangency floating point alone decides whether two avatars touch). Two discs never overlap, and a disc is convex, so the straight line the animation draws between waypoints stays inside it: `roamPath` needs no rejection sampling, no repair pass and no obstacle awareness, because a territory is only handed out if it was already clear. The alternative considered and rejected was a per-frame steering simulation - real avoidance, but JavaScript every frame for up to 200 agents on the audio room's thread. **Accepted cost: roaming range shrinks as the room fills**, because the floor really is being divided between more people. `gridSlot`, `computeGridDims`, `bandPx` and the footprint-overlap tests are gone with the old grid.
...
- **"You" is a follow-spot on the floor, not a ring.** A ring around the avatar read as a bulky square: the svg's artwork is inset from its box, so the band sat detached from the glyph and never hugged it. `.radio-you` pools light under your feet instead, scales with the avatar, and reads at any density. The cheer separately uses `brightness()`/`saturate()` on top of the halo - a bigger blur alone did not look brighter than the neighbours.
